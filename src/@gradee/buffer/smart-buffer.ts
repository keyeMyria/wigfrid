...
class SmartBuffer {
     ...
     *
     * @param byteLength { Number } The byte length of the data that needs to be written.
     * @param offset { Number } The offset of the data to be written.
     */
    private ensureWriteable(byteLength: number, offset?: number) {
        const offsetVal = typeof offset === 'number' ? offset : 0;

        // Ensure there is enough internal Buffer capacity.
        this.ensureCapacity(byteLength + offsetVal);
     ...
     *
     * @param peekOffset { Number } The minimum length of the data needs to be written.
     */
    private ensureCapacity(peekOffset: number) {
        let length = this.buff.length;

        //check buffer whether need to expand
        if (peekOffset > length) {
            let data = this.buff;

            if (length < 128 && peekOffset < 128) {
                length = 128;
            } else {
                while (length < peekOffset) {
                    length <<= 1;
                }
            }
            if (length > 0x20000000) {
                throw new Error('Size of byte to write is out of bound.');
            }

            this.buff = Buffer.allocUnsafe(length);
