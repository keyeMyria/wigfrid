import {DetailMergeManager} from "./DetailMergeManager";
...
import {GroupRow} from "../Grid/Row";
import {isFunction} from "../../../util/isFunction";
import {Column} from "../Grid/Row";
import {CellRangeEventArgs} from "../Grid/CellRangeEventArgs";
import {_NewRowTemplate} from "../Grid/_NewRowTemplate";
...
export class FlexGridDetailProvider {
    ...
    constructor(grid: FlexGrid) {
        ...
        grid.formatItem.subscribe(this._formatItem.bind(this));

        // show details for selected cell
        grid.selectionChanged.subscribe(this._selectionChanged.bind(this));

        // refresh controls to update layout when detail rows are resized
        grid.resizedRow.subscribe(this._resizedRow.bind(this));

        // hide all details when grid is refreshed
        grid.loadingRows.subscribe(() => {
            this.hideDetail();
        });

        // hide detail when dragging row
        grid.draggingRow.subscribe((s, e: CellRangeEventArgs) => {
            this.hideDetail(e.row);
        });

        // keep detail row at the start even with frozen columns (TFS 131863)
        //todo ###fix me###
        grid.formatItem.subscribe((s, e: FormatItemEventArgs) => {
            if (e.panel == s.cells) {
    ...
    showDetail(row: any, hideOthers = false) {
        ...
        if (!this.isDetailVisible(row) && this._hasDetail(row)) {
            ...
            const detailRow  = new DetailRow(rows[row]);
    ...
    _formatItem(s, e: FormatItemEventArgs) {
        ...
        if (e.panel == g.cells && row instanceof DetailRow && row.detail != null) {
            ...
            if (row.height == null) {
                ...
                const cs = getComputedStyle(e.cell);
                let h    = row.detail.scrollHeight + parseInt(cs.paddingTop) + parseInt(cs.paddingBottom);
                ...
                const gridHost = <HTMLElement>row.detail.querySelector('.wj-flexgrid');
        ...
        if (this._mode == DetailVisibilityMode.ExpandMulti ||
            ...
            if (e.panel == g.rowHeaders && e.col == 0 && this._hasDetail(e.row)) {
                ...
                const minus = e.row < g.rows.length - 1 && g.rows[e.row + 1] instanceof DetailRow;
