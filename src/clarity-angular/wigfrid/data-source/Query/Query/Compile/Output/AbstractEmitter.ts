...
export abstract class AbstractEmitterVisitor implements o.StatementVisitor, o.ExpressionVisitor {
    ...
    visitIfStmt(stmt: o.IfStmt, ctx: EmitterVisitorContext): any {
        ...
        ctx.print(`) {`);
        let hasElseCase = stmt.falseCase && stmt.falseCase.length > 0;
    ...
    visitWriteVarExpr(expr: o.WriteVarExpr, ctx: EmitterVisitorContext): any {
        let lineWasEmpty = ctx.lineIsEmpty();
        if (!lineWasEmpty) {
            ctx.print('(');
        }
        ctx.print(`${expr.name} = `);
    ...
    visitLiteralExpr(ast: o.LiteralExpr, ctx: EmitterVisitorContext, absentValue: string = 'null'): any {
        let value = ast.value;
        if (isString(value)) {
    ...
    visitAllObjects(handler: Function, expressions: any, ctx: EmitterVisitorContext, separator: string,
                    newLine: boolean = false): void {
...
export function escapeIdentifier(input: string, escapeDollar: boolean, alwaysQuote: boolean = true): any {
    if (!input) {
        return null;
    }
    const body         = StringWrapper.replaceAllMapped(
